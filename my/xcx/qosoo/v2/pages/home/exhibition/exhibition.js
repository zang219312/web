...
Page({
  ...
  toApply(e) {
    ...
    if (e.currentTarget.dataset.click == 'comfirm') {
      ...
      reqpost(app.globalData.url, '/weixin/api/accept/scan/sc1', {
        ...
        console.log(res);
        if (res.success) {
          this.data.exhibitionlist[this.data.bindex].join = 2;
          this.setData({
            exhibitionlist: this.data.exhibitionlist
          });
          this.hidePopup();

          this.setData({
            showNotify: true,
            notifyText1: this.data.qrcode == 'QR0002' ? '企业参展已报名' : '已报名',
  ...
  registration: function(e) {
    var that = this;
    wx.showActionSheet({
      itemList: ['个人报名', '公司报名'], //0,1
      success(res) {
        let qrcode = res.tapIndex > 0 ? 'QR0002' : 'QR0004';
        that.setData({
          qrcode: qrcode
        })
        let subInfo = JSON.stringify({
          exhibitionlist: that.data.exhibitionlist[e.currentTarget.dataset.index],
          qrcode: qrcode
        })
    ...
    //           console.log(res);
    //           if (res.success) {
    //             that.data.exhibitionlist[e.currentTarget.dataset.bindex].join = 2;
    //             that.setData({
    //               exhibitionlist: that.data.exhibitionlist
    //             });
    //             that.getTheDiscount();
    //           }
    //         })
    ...
    // wx.navigateTo({
    //   url: '/pages/home/applyForRegistration/applyForRegistration?source=regi',
    // })
  },
  sign(e){
    this.setData({
      popup_title:'请在展会期间扫码签到',
