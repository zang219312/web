...
Page({
  ...
  data: {
    items: [
      { name: 'Y', value: '已有公司', checked: 'true' },
      { name: 'N', value: '暂无公司' }
    ],
    companyClassStatus: "",  //注册类型style状态
    isCompany: true,
    val_regType: "Y",
    srk_job: false,
    job_array: "",
    ...
    input_wechat: "",
    input_facebook: "",
    logoImg: '/images/userCenter/icon_camera_1@3x.png',
    logoImgShow: true,
    nickName: '',
    sex: '',
    headImg: "",
...
/*
    ...
    */
    wx.request({
      url: app.globalData.bastUrl + '/industry/list', //仅为示例，并非真实的接口地址
      data: {
  ...
  /******* 获取input的值 *********/
  //获取帐户/手机
  input_accountValue: function (e) {
    this.setData({
      input_account: e.detail.value,
    })
    console.log('输入帐户手机为', e.detail.value)
  },
  //获取用户名
  ...
  hasCompanyReg: function () {
    console.log('有公司的参数：手机号：' + this.data.input_account, '姓名：' + this.data.input_userName, '是否有公司：' + this.data.val_regType)

    if (this.data.input_account == '') {
      wx.showToast({
        title: "请输入手机号码!",
        icon: 'none',
      })
    ...
    reqpost(app.globalData.bastUrl, '/user/register', {
      ...
      code: this.data.val_autherCode,
      openId: wx.getStorageSync('open_id'),
      realName: this.data.input_userName,
      mobile: this.data.input_account,
      headImg: this.data.userInfoDic.avatarUrl,
      sex: this.data.userInfoDic.gender,
      nickName: this.data.userInfoDic.nickName,
    }, "POST", true).then(res => {
  ...
  noCompanyReg: function () {
    console.log('个人参数：手机号：' + this.data.input_account, '姓名：' + this.data.input_userName, '是否有公司：' + this.data.val_regType)
    // 个人注册
    if (this.data.input_account == '') {
      wx.showToast({
        title: "请输入手机号码!",
    ...
    reqpost(app.globalData.bastUrl, '/user/registerNormal', {
      industryId: this.data.job_id,
      code: this.data.val_autherCode,
      openId: wx.getStorageSync('open_id'),
      realName: this.data.input_userName,
...
modifyUser: function(){
  ...
  */
  ...
  getCode:function(){
    ...
    req(app.globalData.bastUrl, '/user/sendRegisterMessage', {
      'mobile': this.data.input_account
    }, "GET", true).then(res => {
      console.log(res);
      wx.showToast({
